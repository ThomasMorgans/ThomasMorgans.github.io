...
{
    int damage = player.attack + (rand() % 6 + 1) - enemy.defence;
    if (damage < 0)
    {
        damage = 0;
    }
...
game character_creator(std::string player_name, std::string race)
{
    if (race == "Human")
    {
        game player(player_name, "player", race, {}, 22, 6, 5);
        return player;    
    }
    if (race == "Elf")
    {
        game player(player_name, "player", race, {}, 20, 6, 5);
        return player;    
    }
    else if (race == "Dwarf")
    {
        game player(player_name, "player", race, {}, 22, 5, 7);
        return player;    
    }
    else if (race == "Halfling")
    {
        game player(player_name, "player", race, {}, 18, 5, 4);
        return player;
    }
}
