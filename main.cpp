...
{ 
    ...
    game tibbles("Mr Tibbles the cat", "A cute and cuddly pet who wants nothing more than an adoring owner.", "pet", {}, 8, 3, 2);
    game clifford("Clifford the Big Red Dog", "Not sure his bark is worse than his bite...", "pet", {}, 30, 8, 4);
    game ex("Your ex", "How has the most bloodthirsty creature of all time wound up down here. This could get ugly.", "", {}, 15, 6, 4);
    game goblin("Goblin git", "A rude and solitary goblin, commonly known to science as a git.", "", {}, 15, 4, 4);
    game teenage_turtle("Teenage Mutant Ninja Turtle", "Cowabunga!", "", {}, 15, 10, 10);
    game rat("Escaped lab rat", "It's HUGE! You wonder what they are feeding these things.", "pet", {}, 6, 3, 2);
    ...
    game giant_spider("Giant Spider", "AHHHH IT'S HUGE SOMEBODY KILL IT!!!", "", {}, 8, 8, 2);
    game bfg("Big Frosty Giant", "God he's mean.", "", {}, 40, 15, 15);
    game troll("Troll", "They have a cave troll!", "", {}, 35, 14, 15);
    game vampire("Vampire", "Is that Robert Pattinson?", "undead", {}, 25, 16, 10);
    game ghost("Ghost", "Spooky... Who you gonna call?", "undead", {}, 8, 3, 1);
    game skeleton("Skeleton", "This guy won't put up much of a fight, he doesn't have the guts.", "undead", {}, 10, 4, 5);
    game werewolf("Werewolf", "What big eyes, ears and teeth he has. Who could possibly mistake this for their grandma.", "", {}, 25, 15, 7);
    game baby_dragon("Baby dragon", "It would be cute if it didn't look so hungry.", "", {}, 10, 5, 5);
    game knight("The Black Knight", "Tis but a scratch. This guy doesn't know when he is beat.", "", {}, 25, 5, 10);
    game unicorn("Unicorn", "A rare and beautiful creature. Its horn is extremely valuable to boot.", "", {}, 25, 5, 2);
    game witch("Sandwitch", "After mixing various potions and herbs, from her cauldron she pulls out... a meatball marina.", "", {}, 15, 10, 4);

    std::vector<game> monsters = {rat, spider, tibbles, ex, goblin, teenage_turtle, ghost, skeleton, baby_dragon, unicorn, witch};
    std::vector<game> boss = {spider, giant_spider, knight, clifford, vampire, werewolf, troll, bfg, dragon};
    ...
    hat, boots, cape, garlic, jacket, codpiece, mask, belt, shield, trousers, chest, chainmail};
    ...
    // Print out map
        ...
        if (map[y][x] == '+')
        {
            ...
            if (player.get_health() > 0 && enemy.get_race() != "pet")
            {
                ...
                else if (item.get_health() == 0 && player.get_inventory().size() < 8)
                {
                ...
                for (int i = 0; i < player.get_inventory().size(); i++)
                {
                    if (item.get_attack() > player.get_inventory()[i].get_attack() > 0 && player.get_inventory().size() == 8)
                    {
                        player.get_inventory()[i] = item;
                        player.change_attack(item.get_attack());
                    }
                    else if (item.get_defence() > player.get_inventory()[i].get_defence() > 0 && player.get_inventory().size() == 8)
                    {
                        player.get_inventory()[i] = item;
                        player.change_defence(item.get_defence());
                    }
            ...
            if (player.get_race() == "Halfling")
            {
                ...
                else if (item_2.get_health() == 0 && player.get_inventory().size() < 8)
                {
                ...
                for (int i = 0; i < player.get_inventory().size(); i++)
                {
                    if (item_2.get_attack() > player.get_inventory()[i].get_attack() > 0 && player.get_inventory().size() == 8)
                    {
                        player.get_inventory()[i] = item_2;
                        player.change_attack(item_2.get_attack());
                    }
                    else if (item_2.get_defence() > player.get_inventory()[i].get_defence() > 0 && player.get_inventory().size() == 8)
                    {
                        player.get_inventory()[i] = item_2;
                        player.change_defence(item_2.get_defence());
                    }
        ...
        for (int i = 0; i < player.get_inventory().size(); i++)
        {
            if (player.get_inventory()[i].get_name() == player.get_inventory()[player.get_inventory().size() - 1].get_name() && i != player.get_inventory().size() - 1)
            {
                std::cout << "You can't have two!";
                player.change_attack(-(player.get_inventory()[player.get_inventory().size() - 1].get_attack()));
                player.change_defence(-(player.get_inventory()[player.get_inventory().size() - 1].get_defence()));
