...
{ 
    ...
    game witch("Sandwitch", "After mixing various potions and herbs, from her cauldron she pulls out... a meatball marinara.", "", {}, 15, 12, 3);
        ...
        while (player.inventory.size() > 8)
        {
            ...
            for (int i = 0; i < player.inventory.size(); i++)
            {
                if (player.inventory[i].name == drop)
                {
                    ...
                    for (int j = player.inventory.size() - 1; j > i; j--)
                    {
